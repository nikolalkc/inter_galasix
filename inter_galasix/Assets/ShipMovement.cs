...
public class ShipMovement : MonoBehaviour
{
	...
	void Start ()
	{
		TurboStats = GameObject.Find ("TurboStat").GetComponent<Slider> ();
		main = Camera.main;
	...
	void Update ()
	{
		...
		if (Input.GetButton ("Boost")) {
			CancelInvoke("RechargeNitro");
			TurboStats.value -= 1;
			if (TurboStats.value > 0) {
				rig.AddForce (transform.up * forceBoost);
			}
		} 
		// trosi nitro
		else {
			if (TurboStats.value < 100) {
				Invoke ("RechargeNitro",3f);
			}
		}
